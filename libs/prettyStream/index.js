...
PrettyStream.prototype.write = function(obj) {
  var level = this.levels[obj.level];
  ...
  if(obj.req) {
    ...
    console.log('[%s] ' + this.colors[color]('%s') + ' %s %s %s ms', obj.time.toUTCString(), obj.res.statusCode, obj.req.method, obj.req.url, obj['response-time'])
  } else {
    if(obj.err) {
      // Log errors and their stacks.
      console.log('[%s] ' + this.colors[level.color]('[%s]') + ': ' + this.colors[level.color]('%s') + '\n' + this.colors[level.color]('%s'),
